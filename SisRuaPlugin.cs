    ...
    public class SisRuaPlugin : IExtensionApplication
    {
        ...
        public void Initialize()
        {
            ...
            try
            {
                ...
                pythonExePath = EnsureVenvAndDependencies(projectRoot, pythonExePath);
                if (string.IsNullOrEmpty(pythonExePath))
                {
        ...
        private string FindProjectRoot(string startPath)
        {
            ...
            while (currentDir != null && sanityCheck < 10)
            {
                ...
                // (ex.: repo raiz contém sisRUA.bundle\Contents\backend)
                string bundleContents = Path.Combine(currentDir.FullName, "sisRUA.bundle", "Contents");
                if (Directory.Exists(Path.Combine(bundleContents, "backend")))
                {
        ...
        private string EnsureVenvAndDependencies(string projectRoot, string pythonExePath)
        {
            try
            {
                string requirementsPath = Path.Combine(projectRoot, "backend", "requirements.txt");
                ...
                if (!TryPythonImport(venvPython, "fastapi,uvicorn,osmnx,pyproj,shapely", projectRoot))
                {
                    ...
                    RunProcess(venvPython, projectRoot, timeoutMs: 10 * 60 * 1000, "-m", "pip", "install", "--upgrade", "pip");
                    ...
                    if (!TryPythonImport(venvPython, "fastapi,uvicorn,osmnx", projectRoot))
                    {

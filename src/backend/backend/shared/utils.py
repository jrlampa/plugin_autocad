    ...
    for line in lines:
        ...
        for (x, y) in projected.coords:
            ...
            if not math.isfinite(fx) or not math.isfinite(fy):
                continue
            # CAD Safety: Prevent overflow in AutoCAD which can cause fatal errors
            if abs(fx) > 1e8 or abs(fy) > 1e8:
                continue
            coords.append([fx, fy])
        if len(coords) >= 2:
            out.append(coords)
...
def get_layer_config() -> Dict[str, Any]:
    ...
    if layers_path.exists():
        try:
            ...
            get_logger(__name__).error("json_load_failed", error=str(e), path=str(layers_path))

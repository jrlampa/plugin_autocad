...
# ---------------------------------------------------------------------------
# Testes ABNT — conformidade com NBR 14166:1998 e NBR 13133:2021
# ---------------------------------------------------------------------------
...
def test_abnt_escala_str():
    ...
    meta = AbntDrawingMetadata(escala=1_000)
    assert meta.escala_str() == "1:1.000"

    meta2 = AbntDrawingMetadata(escala=25_000)
    assert meta2.escala_str() == "1:25.000"
...
def test_abnt_tolerancia_por_escala():
    ...
    # 1:1.000 → tolerância = 0.20 m (0,2 mm × 1000)
    assert AbntDrawingMetadata(escala=1_000).tolerancia_m() == pytest.approx(0.20)
    # 1:500 → tolerância = 0.10 m
    assert AbntDrawingMetadata(escala=500).tolerancia_m() == pytest.approx(0.10)
    # 1:10.000 → tolerância = 2.00 m
    assert AbntDrawingMetadata(escala=10_000).tolerancia_m() == pytest.approx(2.00)
...
def test_abnt_nearest_escala():
    """nearest_abnt_escala deve retornar a escala ABNT mais próxima."""
    from backend.gis_core.abnt import nearest_abnt_escala

    assert nearest_abnt_escala(800) == 1_000   # mais próximo de 500 ou 1000
...
def test_abnt_build_default_metadata_epsg():
    """build_default_metadata deve derivar zona e label do EPSG."""
...
def test_dxf_with_abnt_metadata(tmp_path):
    """DXF exportado com metadados ABNT deve ser válido e conter FINGERPRINTGUID."""
    import ezdxf
    from backend.gis_core.abnt import AbntDrawingMetadata
    ...
    features = [_make_road("Via ABNT", "SISRUA_OSM_HIGHWAY", length_m=100.0)]
    out = tmp_path / "test_abnt.dxf"
    export_features_to_dxf(features, output_path=out, metadata=meta)

    doc = ezdxf.readfile(str(out))
    assert doc is not None
    # Verifica que o DXF é válido R2010+
    assert doc.dxfversion >= "AC1024"
    # Verifica FINGERPRINTGUID com metadados sisRUA
    fingerprint = doc.header.get("$FINGERPRINTGUID", "")
    assert "sisrua" in fingerprint
    assert "23S" in fingerprint


def test_dxf_without_explicit_metadata_uses_default(tmp_path):
    """export_features_to_dxf sem metadados explícitos deve usar build_default_metadata."""
    import ezdxf
    from backend.services.dxf_export import export_features_to_dxf

    features = [_make_road(length_m=500.0)]
    out = tmp_path / "test_default_meta.dxf"
    # Sem metadata= → build_default_metadata(epsg=31983) internamente
    export_features_to_dxf(features, output_path=out, epsg=31983)
